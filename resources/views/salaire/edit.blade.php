                ...
                <div class="col-12 col-md-8">
                    <div class="app-card app-card-settings shadow-sm p-4">
                        ...
                        <div class="app-card-body">
                            <form class="settings-form" action={{ route('salaire.update', $salaire->id) }} method="PUT">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                    ...
                                    <select name="employe_id" id="employe_id" class="form-control">
                                        @foreach ($employes as $employe)
                                            <option value={{ $employe->id }}
                                                {{ $salaire->employe->id == $employe->id ? 'selected' : '' }}>
                                                {{ $employe->nom }}</option>
                                        @endforeach
                                        @error('employe_id')
                                ...
                                <div class="row mb-3 d-flex">
                                    <div class="col-md-11">
                                        ...
                                        <input type="number" class="form-control" id="setting-input-1"
                                            placeholder="Saisir le Montant" name="montant" value={{ $salaire->montant }}>
                                        @error('montant')
                                            <p class="text-danger" style="font-size: 13px;">{{ $message }}</p>
                                        @enderror
                                    </div>
